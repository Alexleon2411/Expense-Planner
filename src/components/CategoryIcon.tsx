...
export default function CategoryIcon({ icon, color, name, size = 'md', className = '' }: Props) {
  ...
  if (icon) {
    ...
    return (
      <span
        className={`material-symbols-outlined ${iconSize} rounded-xl flex items-center justify-center ${className}`}
        style={{ color: color || '#3b82f6', backgroundColor: color ? `${color}20` : '#3b82f620' }}
      >
        {icon}
      </span>
    )
